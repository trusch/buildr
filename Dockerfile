...
RUN apt-get update && apt-get install -y make gcc g++ automake curl unzip xz-utils ca-certificates apt-transport-https gnupg pass
...
ENV PATH /opt/nodejs/bin:/opt/go/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
ENV GOROOT /opt/go
ENV GOPATH /go
